...
-- Profiles (created first so policies referencing it can be created safely)
...
-- Disable RLS for flowers table: Admin interface is protected by passcode authentication.
-- Hidden flowers are filtered at the application layer (see data.ts fetchFlowers function).
-- This allows the admin panel to freely update flower properties without RLS conflicts.
ALTER TABLE flowers DISABLE ROW LEVEL SECURITY;
