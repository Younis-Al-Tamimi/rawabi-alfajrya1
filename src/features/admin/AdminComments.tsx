...
export function AdminComments() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="font-serif text-3xl text-forest-700 mb-1">{t('commentsModeration')}</h1><p className="text-forest-400 text-sm">{t('commentsModerationSubtitle')}</p></div>
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]"><SearchIcon size={18} className="absolute top-1/2 -translate-y-1/2 start-3 text-forest-400" /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t('searchComments')} className="input-field ps-10" /></div>
        <select value={flowerFilter} onChange={(e) => setFlowerFilter(e.target.value)} className="input-field w-auto"><option value="all">{t('allFlowers')}</option>{flowers.map(f => <option key={f.id} value={f.id}>{getFlowerName(f, lang)}</option>)}</select>
      </div>
            ...
            <motion.div key={comment.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className={`card p-4 ${comment.is_hidden ? 'opacity-60' : ''}`}>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap"><p className="font-sans text-sm font-medium text-forest-700">{comment.profiles?.display_name || 'Anonymous'}</p><p className="text-xs text-forest-400">{timeAgo(comment.created_at, lang)}</p>{comment.is_hidden && <span className="text-xs bg-forest-100 text-forest-500 px-2 py-0.5 rounded-full">{t('hidden')}</span>}</div>
                  <p className="text-forest-600 text-sm mb-2">{comment.text}</p>
                  <div className="flex items-center gap-3 text-xs text-forest-400">{comment.flower && <Link to={`/admin/flowers/${comment.flower.id}`} className="hover:text-gold-500 transition-colors">{getFlowerName(comment.flower, lang)}</Link>}<span>·</span><span>{comment.like_count} {t('likes')}</span></div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => handleToggleHidden(comment)} className={`p-2 rounded-lg transition-colors ${comment.is_hidden ? 'text-forest-500 hover:bg-forest-50' : 'text-gold-600 hover:bg-gold-50'}`} title={comment.is_hidden ? t('unhide') : t('hide')}>{comment.is_hidden ? <EyeIcon size={18} /> : <EyeOffIcon size={18} />}</button>
