-- Add missing email and password_hash to profiles and populate from auth.users
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'profiles_email_key') THEN
    ...
    EXCEPTION WHEN duplicate_object THEN
      -- constraint already exists in another form, ignore
      NULL;
    END;
  END IF;
END$$;
...
-- Recreate trigger to ensure future auth.user inserts create profiles including email
...
-- Optional: ensure password_hash column exists and is nullable for compatibility
